...
public class TJImplementation {
    ...
    private static Method CraftItemStack_asBukkitCopy;

    private static Method NMSItemStack_getOrCreateNBT;
    private static Method NMSItemStack_setNBT;

    private static Constructor<?> NBTTagCompound_new;
    private static Method NBTTagCompound_getNBTBase;
    private static Method NBTTagCompound_setNBTBase;

    private static Constructor<?> NBTTagList_new;
    private static Method NBTTagList_size;
    private static Method NBTTagList_set;
    private static Method NBTTagList_add;

    private static Method NBTTagString_new;

    static {
        if (client_language) try {
            var CraftItemStack = Class.forName("org.bukkit.craftbukkit." + Version.version + ".inventory.CraftItemStack");
            CraftItemStack_asNMSCopy = CraftItemStack.getMethod("asNMSCopy", ItemStack.class);
            //
            var NMSItemStack = Class.forName("net.minecraft.world.item.ItemStack");
            CraftItemStack_asBukkitCopy = CraftItemStack.getMethod("asBukkitCopy", NMSItemStack);
            //
            NMSItemStack_getOrCreateNBT = NMSItemStack.getMethod(Version.getTarget(new HashMap<>() {{
                put(1191, "v");
                put(1181, "t");
                put(1171, "getOrCreateTag");
            }}));
            var NBTTagCompound = Class.forName("net.minecraft.nbt.NBTTagCompound");
            NBTTagCompound_new = NBTTagCompound.getConstructor();
            NMSItemStack_setNBT = NMSItemStack.getMethod(Version.getTarget(new HashMap<>() {{
                put(1181, "c");
                put(1171, "setTag");
            }}), NBTTagCompound);
            //
            NBTTagCompound_setNBTBase = NBTTagCompound.getMethod(Version.getTarget(new HashMap<>() {{
                put(1181, "a");
                put(1171, "set");
            }}), String.class, Class.forName("net.minecraft.nbt.NBTBase"));
            NBTTagCompound_getNBTBase = NBTTagCompound.getMethod(Version.getTarget(new HashMap<>() {{
                put(1181, "c");
                put(1171, "get");
            }}), String.class);
            //
            var NBTTagList = Class.forName("net.minecraft.nbt.NBTTagList");
            NBTTagList_new = NBTTagList.getConstructor();
            NBTTagList_size = NBTTagList.getMethod("size");
            NBTTagList_set = NBTTagList.getMethod("set", int.class, Object.class);
            NBTTagList_add = NBTTagList.getMethod("add", Object.class);

            var NBTTagString = Class.forName("net.minecraft.nbt.NBTTagString");
            NBTTagString_new = NBTTagString.getMethod("a", String.class);
        } catch (ClassNotFoundException | NoSuchMethodException e) {
            Main.ShulkerBoxPreview.getLogger().severe("Something went wrong :( Fallback-ed to to Lang File mode, please report to developer.");
            e.printStackTrace();
            client_language = false;
        }
    }

    public static ItemStack asLoreSet(ItemStack item, List<BaseComponent> components) {
        try {
            var nms = CraftItemStack_asNMSCopy.invoke(null, item);
            var nbt = NMSItemStack_getOrCreateNBT.invoke(nms);
            var display = NBTTagCompound_getNBTBase.invoke(nbt, "display");
            if (display == null) display = NBTTagCompound_new.newInstance();
            var list = NBTTagCompound_getNBTBase.invoke(display, "Lore");
            if (list == null) list = NBTTagList_new.newInstance();
            var serialized = components.stream().peek(e -> {
                if (e.isItalicRaw() == null) e.setItalic(false);
            }).map(ComponentSerializer::toString).toList();
            for (int i = 0; i < serialized.size(); i++) {
                var nbtTagString = NBTTagString_new.invoke(null, serialized.get(i));
                if (i > (int) NBTTagList_size.invoke(list) - 1) NBTTagList_add.invoke(list, nbtTagString);
                else NBTTagList_set.invoke(list, i, nbtTagString);
            }
            NBTTagCompound_setNBTBase.invoke(display, "Lore", list);
            NBTTagCompound_setNBTBase.invoke(nbt, "display", display);
            NMSItemStack_setNBT.invoke(nms, nbt);
            return (ItemStack) CraftItemStack_asBukkitCopy.invoke(null, nms);
    ...
    public static class ComponentProcessor {
        public static BaseComponent join(BaseComponent a, BaseComponent... bs) {
            for (BaseComponent b : bs) a.addExtra(b);
            return a;
        }
        ...
        public static String refreshFormat(String now) {
            StringBuilder s = new StringBuilder();
            var array = now.toCharArray();
            for (int i = 0; i < array.length; i++) {
                ...
                if (i + 1 < array.length) {
                    ...
                    switch (next) {
                        ...
                        default -> {
                            var code = ChatColor.getByChar(next);
        ...
        private static void applyFormatOrColor(BaseComponent component, char formatOrColorChar) {
            ChatColor color = ChatColor.getByChar(formatOrColorChar);
            if (color.equals(ChatColor.RESET)) {
                resetFormatting(component);
            } else if (color.getColor() == null) {
                if (color.equals(ChatColor.BOLD)) component.setBold(true);
                else if (color.equals(ChatColor.ITALIC)) component.setItalic(true);
                else if (color.equals(ChatColor.UNDERLINE)) component.setUnderlined(true);
                else if (color.equals(ChatColor.STRIKETHROUGH)) component.setStrikethrough(true);
                else if (color.equals(ChatColor.MAGIC)) component.setObfuscated(true);
            } else {
                component.setColor(color);
            }
        }

        private static void resetFormatting(BaseComponent component) {
            component.setBold(false);
            component.setItalic(false);
            component.setUnderlined(false);
            component.setStrikethrough(false);
            component.setObfuscated(false);
            component.setColor(ChatColor.WHITE); // Resetting to default color
        }
    }
    ...
    private static class Version {
        ...
        private static <T> T getTarget(Map<Integer, T> data) {
            TreeMap<Integer, T> sortedData = new TreeMap<>(data);
            Map.Entry<Integer, T> entry = sortedData.floorEntry(versionNumber);

            if (entry == null) throw new IllegalStateException("Sorry, not implemented yet: " + version + "\n" + data);
    ...
    public static class LocaleManager {
        ...
        private static final boolean oldVersion;
        ...
        private static final Map<String, String> oldBlocks = LocaleKeys.getBlockKeys();
        private static final Map<String, String> oldItems = LocaleKeys.getItemKeys();
        private static final Map<String, String> oldPotions = LocaleKeys.getPotionKeys();
        private static final Map<String, String> oldLingeringPotions = LocaleKeys.getLingeringPotionKeys();
        private static final Map<String, String> oldSplashPotions = LocaleKeys.getSplashPotionKeys();
        private static final Map<String, String> oldEntities = LocaleKeys.getEntityKeys();
        ...
        public String queryEntityType(final EntityType entityType, final String extra) {
            if (entityType == null) {
                throw new NullPointerException("[LocaleLib] EntityType cannot be null");
            }
            String key = "";
            if (oldVersion) {
                if (entityType.name().equals("VILLAGER") && extra != null && Villager.Profession.valueOf(extra) != null) {
                ...
                } else if (entityType.name().equals("VILLAGER") && extra != null && Villager.Profession.valueOf(extra) != null) {
        ...
        public static String queryItemStack(final ItemStack itemStack) {
            return queryMaterial(itemStack.getType(), itemStack.getDurability(), itemStack.getItemMeta());
        }

        @SuppressWarnings("deprecation")
        public static String queryMaterial(final Material material, final short durability, final ItemMeta meta)
        ...
        private String translate(final String message, final String key, final String placeholder) {
            ...
            if (text.contains("§")) {
                ...
                if (matcher.find()) {
                    ...
                    final org.bukkit.ChatColor color = org.bukkit.ChatColor.getByChar(org.bukkit.ChatColor.getLastColors(text).replace("§", ""));
                    if (color != null) {
        ...
        private static boolean _isBelow113(final String bukkitVersion) {
            if (bukkitVersion.matches("^[0-9.]+$")) {
                return switch (bukkitVersion) {
                    case "1.12.2", "1.12.1", "1.12", "1.11.2", "1.11.1", "1.11", "1.10.2", "1.10.1", "1.10", "1.9.4", "1.9.3", "1.9.2", "1.9.1", "1.9", "1.8.9", "1.8.8", "1.8.7", "1.8.6", "1.8.5", "1.8.4", "1.8.3", "1.8.2", "1.8.1", "1.8", "1.7.10", "1.7.9", "1.7.2" ->
                            true;
                    default ->
